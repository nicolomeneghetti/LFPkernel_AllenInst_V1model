...
NEURON_TYPE_ID = os.listdir('./kernels/flasehs/e4other/') # the folders where the kernels are stored
NUM_LEVELS = 2



spikes_tables_V1 = pd.read_pickle('./Allen_Inst_V1_model_features/L4_spiking_flashes/spike_fullcolumn_L4_flashes.pkl')
...
def process_target_folder(folder, target, t_axis):
    ...
    for outer_idx, subfolder in enumerate(subfolders, 1):
        ...
        with open('./inputs to estimate LFP from kernels/flashes/mem_potential_dict.pkl', 'rb') as file:
            mem_potential_dict = pickle.load(file)
        mem_potentials = mem_potential_dict[subfolder]
        ...
        # Load vector levels
        vector_levels_path = './kernels/flashes/'+str(target)+'/'+str(subfolder)+'/vector_time_normalized_levels'
...
def main():
    ...
    for target in TARGET_VECTOR:    
        ...
        folder = './kernels/flashes/' #the folder in which the kernels are stored
...
for outer_loop, folder_key in enumerate(chiavi, start=1):
    ...
    output_file = f"./from kernels to LFPs/flasehs/LFPs_{folder_key}_e4other.p"
...
NEURON_TYPE_ID = os.listdir('./kernels/flashes/e4other/') # the folders where the kernels are stored
...
for ind1 in NEURON_TYPE_ID: #the target ID of the e4other family in the single-layer L23 model
    # Load LFP data
    lfp_path = f"./from kernels to LFPs/flashes/LFPs_{ind1}_e4other.p"
    LFP_kernels = pickle.load(open(lfp_path, "rb"))
    
    
    with open('./from kernels to LFPs/flashes/LFP_allen_dict.pkl', 'rb') as file:
