...
from Allen_Inst_V1_model_features.utils_functions import (filter_connectivity, load_dataframe, compute_population_parameters,
...
def compute_utils_from_bmtk_to_population(target_population: str, target_node_id: int, simulation_length: int, dt_simulation: float, zaxis_rotation: float, num_levels:int):
    ...
    connectivity_files = {
        "V1-V1": './Allen_Inst_V1_model_features/features_V1_V1_L4.pkl',
        "LGN-V1": './Allen_Inst_V1_model_features/features_LGN_V1_L4.pkl',
        "BKG-V1": './Allen_Inst_V1_model_features/features_BKG_V1_L4.pkl',
        "FB-V1": './Allen_Inst_V1_model_features/features_FB_V1_L4.pkl',
    }
       
    
    connectivity_L4V1_to_L4V1 = filter_connectivity(load_dataframe(connectivity_files['V1-V1']), target_population, target_node_id)
    
    connectivity_LGN_to_L4V1 = filter_connectivity(load_dataframe(connectivity_files['LGN-V1']), target_population, target_node_id)
    connectivity_BKG_to_L4V1 = filter_connectivity(load_dataframe(connectivity_files['BKG-V1']), target_population, target_node_id)
    connectivity_FB_to_L4V1 = filter_connectivity(load_dataframe(connectivity_files['FB-V1']), target_population, target_node_id)



    X = list(connectivity_L4V1_to_L4V1['source_query'])+\
    list(connectivity_LGN_to_L4V1['LGN_pop_name'])+\
    list(connectivity_BKG_to_L4V1['source_query'])+\
    list(connectivity_FB_to_L4V1['source_query']) # contains the list of the presynaptic families

    N_X = np.array(list(connectivity_L4V1_to_L4V1['numerosity_source_nodes'])+\
                   list(connectivity_LGN_to_L4V1['numerosity_source'])+\
                   list(connectivity_BKG_to_L4V1['numerosity_source_nodes'])+\
                   list(connectivity_FB_to_L4V1['numerosity_source_nodes'])) # presynpatic population sizes


    Y = target_population # postsynaptic population
    N_Y = int(list(connectivity_L4V1_to_L4V1['numerosity_target_nodes'])[0]) # postsynaptic population size

    g_eff = True  # account for changes in passive leak due to persistent synaptic activations


    C_YX = list(connectivity_L4V1_to_L4V1['conn_probability'])+\
    list(connectivity_LGN_to_L4V1['conn_probability'])+\
    list(connectivity_BKG_to_L4V1['conn_probability'])+\
    list(connectivity_FB_to_L4V1['conn_probability'])# contains the list of connection probabilities of the presynaptic families
    ...
    node_types = pd.read_csv(os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'v1_node_types.csv'), sep=' ')
    ...
    dynamics_dir = os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'biophys_components', 'biophysical_neuron_templates')
    ...
    somatic_positions = load_dataframe(os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'posizione_somatiche.pkl'))
    ...
    synaptic_positions_V1_to_V1 = pd.read_pickle('./Allen_Inst_V1_model_features/synaptic_positions/V1_to_e4other_synaptic_positions.pkl')    
    
    synaptic_positions_LGN_to_V1 = pd.read_pickle('./Allen_Inst_V1_model_features/synaptic_positions/LGN_to_e4other_synaptic_positions.pkl')

    synaptic_positions_BKG_to_V1 = pd.read_pickle('./Allen_Inst_V1_model_features/synaptic_positions/BKG_to_e4other_synaptic_positions.pkl')
    synaptic_positions_BKG_to_V1['source'] = 'BKG'

    synaptic_positions_FB_to_V1 = pd.read_pickle('./Allen_Inst_V1_model_features/synaptic_positions/FB_to_e4other_synaptic_positions.pkl')
    ...
    for _, vector in connectivity_L4V1_to_L4V1.iterrows():
    ...
    for _, vector in connectivity_LGN_to_L4V1.iterrows():
    ...
    if(len(connectivity_BKG_to_L4V1)): 
        mult_params.append(add_mult_params(connectivity_BKG_to_L4V1['nsyns_list_from_build'].item(), 0.05))
        delay_params.append(add_delay_params(connectivity_BKG_to_L4V1['delay'].item()))
        
        syn_type = get_synapse_type(connectivity_BKG_to_L4V1['model_template'].item())
        params_syn = load_synaptic_params(connectivity_BKG_to_L4V1['dynamics_params'].item())
        
    
        weight = handle_missing_weight(connectivity_BKG_to_L4V1['syn_weight'].item())
        synapse_params.append({'weight': weight, 'syntype': syn_type, **params_syn})
        
        pos_args = process_synaptic_positions(connectivity_BKG_to_L4V1.iloc[0], synaptic_positions_BKG_to_V1, default_synapse_pos_args)
        synapse_pos_args.append(pos_args if pos_args else default_synapse_pos_args)

    ###########################################################################
    # Process synaptic features and positions from FB to L23
    if(len(connectivity_FB_to_L4V1)): 
        mult_params.append(add_mult_params(connectivity_FB_to_L4V1['nsyns_list_from_build'].item(), 0.05))
        delay_params.append(add_delay_params(connectivity_FB_to_L4V1['delay'].item()))
        
        syn_type = get_synapse_type(connectivity_FB_to_L4V1['model_template'].item())
        params_syn = load_synaptic_params(connectivity_FB_to_L4V1['dynamics_params'].item())
        
    
        weight = handle_missing_weight(connectivity_FB_to_L4V1['syn_weight'].item())
        synapse_params.append({'weight': weight, 'syntype': syn_type, **params_syn})
        
        pos_args = process_synaptic_positions(connectivity_FB_to_L4V1.iloc[0], synaptic_positions_FB_to_V1, default_synapse_pos_args)
        synapse_pos_args.append(pos_args if pos_args else default_synapse_pos_args)
    ...
    spikes_tables = pd.read_pickle('./Allen_Inst_V1_model_features/L4_spiking_flashes/spike_fullcolumn_L4_flashes.pkl')
    spikes_tables_LGN = pd.read_pickle(os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'L23_flashes', 'spike_lgn_flashes.pkl'))
    spikes_tables_BKG = pd.read_pickle(os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'L23_flashes', 'spike_bkg_flashes.pkl'))
    spikes_tables_FB = pd.read_pickle(os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'L23_flashes', 'spike_fb_flashes.pkl'))
...
def compute_LFP_bmtk_to_population_fromV1(target_population, target_query_node_type_id, zaxis_rotation, num_levels):
    ...
    simulation_length = 7.5 # seconds of simulations when V1 was presented with gratings
...
connectivity_V1_V1 = pd.read_pickle( './Allen_Inst_V1_model_features/features_V1_V1_L4.pkl')

string_target_vector = ['e4other'] # this code is meant to estimate LFP generated by pyramidal cells in L23
...
for index_target in range(len(string_target_vector)):
    ...
    rotazioni_info = json.load(open(os.path.join(parent_dir, 'full_column_layer23', 'Allen_Inst_V1_model_features', 'v1_node_models.json'), 'r'))
