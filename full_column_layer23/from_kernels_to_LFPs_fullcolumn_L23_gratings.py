...
spikes_tables_V1 = pd.read_pickle('./Allen_Inst_V1_model_features/L23_gratings/spike_fullcolumn_L23_gratings.pkl')
spikes_tables_LGN = pd.read_pickle('./Allen_Inst_V1_model_features/L23_gratings/spike_lgn_gratings.pkl')
spikes_tables_BKG = pd.read_pickle('./Allen_Inst_V1_model_features/L23_gratings/spike_bkg_gratings.pkl')
spikes_tables_FB = pd.read_pickle('./Allen_Inst_V1_model_features/L23_gratings/spike_fb_gratings.pkl')
...
def main():
    ...
    h5_path = './inputs_to_estimate_LFPs/mem_e23Cux2_487661754_gratings.h5'
    ...
    # Load vector levels
    vector_levels_path = './kernels/gratings/vector_time_normalized_levels'
...
for outer_loop, folder_key in enumerate(chiavi, start=1):
    ...
    output_file = f"./from kernels to LFPs/gratings/LFPs_{folder_key}_e23Cux2.p"
...
for ind1 in ['487661754']: #the target ID of the e23Cux2 family in the single-layer L23 model
    # Load LFP data
    lfp_path = f"./from kernels to LFPs/gratings/LFPs_{ind1}_e23Cux2.p"
    LFP_kernels = pickle.load(open(lfp_path, "rb"))
    
    
    lfp_path = f"./from kernels to LFPs/gratings/LFPs_Allen_Inst_full_column_e23Cux2.pkl"
    LFP_allen = pickle.load(open(lfp_path, "rb"))
